...
function renderizar() {
    ...
    // Ocultar bloques cuyos campos están vacíos
    const bloquesOpcionales = [
        { id: 's2-fecha', valor: estado.fecha },
        { id: 's2-horario', valor: (estado.desde || estado.hasta) },
        { id: 's2-lugar', valor: estado.lugar },
        { id: 's2-direccion', valor: estado.direccion }
    ];

    bloquesOpcionales.forEach(({ id, valor }) => {
        const el = document.getElementById(id);
        if (!el) return;
        const contenedor = el.closest('.datos, .hora') || el;
        contenedor.classList.toggle('oculto', !valor);
    });
}
...
async function exportarSlide() {
    await document.fonts.ready;  // espera a que carguen WorkFaAAD/WorkSans/NectoMono
    ...
    try {
        ...
        alert('Error al exportar PNG. Revisá la consola.');
    }
}
